...
USED void lbl_DashPanelSpeed(f32 dashPanelSpeed, Player *player, Object *object) {
    ...
    switch (stage) {
        ...
        case EggFactory:
        {
            ...
            else if ((object_id >= 0xEE && object_id <= 0xF8) || (object_id >= 0xFA && object_id <= 0xFD) || 
            (object_id >= 0xDC && object_id <= 0xE0))
            {
        ...
        case SplashCanyon:
            if (object_id >= 0x205 && object_id <= 0x206) {
                dashPanelSpeed += pSpeed(300.0f);
            }
            else if (object_id >= 0x1FB && object_id <= 0x1FC) {
                dashPanelSpeed += pSpeed(50.0f);
            }
            break;
        case RedCanyon:
    ...
    if (magneticImpulseData[player->index].magneticImpulse) {
        ...
        if (!(player->typeAttributes & AllType)) isNoType = MI_NoTypeMultiplier;
        else isNoType = 1.0f;
