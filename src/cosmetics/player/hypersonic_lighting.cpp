...
void HyperSonic_RainbowLighting(Player &player) {
    // This function is now unused, as this animation has been incorporated into a material animation file in the Hyper Sonic model file.
	...
	void *lightingptr        = gpsaObject_Player[index]->vtxListPtrList;
	...
	if(player.hyperSonicInit == 0u) {
		...
		colorState.state            = HyperSonic_ColorState::red;
		colorState.transitionFrames = 0;
	}

	if(gu32GameCnt % 8 == 0) { // every 8 frames
		colorState.state = static_cast<HyperSonic_ColorState::color>(colorState.state + 1);
		colorState.transitionFrames = 0;
	...
	switch(colorState.state) {
		using enum HyperSonic_ColorState::color;
		case red: {
