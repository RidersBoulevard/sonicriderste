...
void Player_LCancelHandler(Player *player) {
	...
	if(player->state < FrontflipRamp || player->state > TurbulenceTrick) { return; }
	if(!player->input->toggleFaceButtons.hasAny(LButton, RButton) || info.lockoutTimer != 0) { return; }
...
ASMUsed u32 Player_CheckLCancel(Player *player, u32 currentTrickStatus) {
    ...
    if (info.frameWindow > 0) {
        if (currentTrickStatus == 2 && !player->specialFlags.hasAny(ringGear)) {
