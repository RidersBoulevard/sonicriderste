...
void Player_AdvantageP(Player &player) {
    ...
    if (player.state == PlayerState::StartLine) {
        ...
        switch (static_cast<Buttons>(buttons)) {
    ...
    switch (advpInfo.TOP_Pos)
    {
        case 1:
            ...
            advpInfo.TOPGauge[1] = player.gearStats[player.level].maxAir * 2/5; // 1/3
            break;
        ...
        case 3:
            advpInfo.TOPGauge[0] = player.gearStats[player.level].maxAir * 3/5; // 2/3
