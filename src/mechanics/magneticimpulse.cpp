	...
	if (player->grindRailDash == 1) {
		if (player->specialFlags.hasAny(SpecialFlags::ringGear)) {
			...
			if (player->character == Character::SuperSonic
				...
				const s32 newBlastGaugeValue = bgInfo->currentGauge + static_cast<s32>(GrindRailDashAirCosts[2] * 1.6);
...
ASMUsed f32 Player_MITrickAirGain(Player* player, f32 airGain){
	...
	if (player->isBlastGaugeGear) {
		...
		// player->gearExload().exLoadID == EXLoad::HyperSonic && hsInfo.hyperdriveEnabled 
		// 	|| 
		if (isSuperCharacter(*player, Character::Shadow)
			|| isSuperCharacter(*player, Character::Tails)) {
			//player->shortcutAirGainMultiplier += player->trickCount * 0;
			//}else if(exLoads.gearExLoadID == HyperSonic && !hsInfo->hyperdriveEnabled){
			//	player->shortcutAirGainMultiplier += player->trickCount * 10000;
			//}else if(exLoads.gearExLoadID != HyperSonic && player->character == SuperSonic){
			//	player->shortcutAirGainMultiplier += player->trickCount * 10000;
		} else {
			// player->shortcutAirGainMultiplier += player->trickCount * 10000;
		...
		// if (player->gearExload().exLoadID == EXLoad::DarkSonic && player->level == 2 && bgInfo->currentGauge >= 100000) {
...
ASMUsed void Player_TrickPayout(Player* player, s32 payoutAmount) {
	...
	switch (player->extremeGear) {
		case ExtremeGear::TheCrazy:
			newRings = player->currentAir + payoutAmount;
			player->currentAir = clamp(newRings, 0, player->gearStats[0].maxAir);
			break;
		case ExtremeGear::AdvantageF:
			newRings = player->currentAir + (payoutAmount / 13);
			player->currentAir = clamp(newRings, 0, player->gearStats[0].maxAir);
			break;
	}
}
...
ASMUsed f32 Player_MITrickLandingSpeed(Player* player, f32 currentTrickSpeed){
	if (player->trickRank > 0x5 && player->extremeGear != ExtremeGear::Legend && player->characterArchetype != CharacterArchetype::Trickster) {
		// S and C rank
		currentTrickSpeed += ((f32)(player->trickCount - (u8)4) * pSpeed(2.0f));
	}

	if (player->extremeGear == ExtremeGear::Legend || player->gearExload().exLoadID == EXLoad::TheProfessional) {
	...
	// 	if (spfInfo->gearChange >= 2 && player->level >= 2) {currentTrickSpeed += pSpeed(30.0f);}
	// }
...
void Player_MagneticImpulseTimer(Player&player){
	HyperSonicInfo* hsInfo = &PlayerHyperSonicInfo[player.index];
	if (hsInfo->hyperdriveEnabled && player.gearExload().exLoadID == EXLoad::HyperSonic) {
		MI::createGuaranteedMaxMIParticles(player);
	}
