...
inline void Player_CoverF_UpdateStats(Player &player, const u8 archetype) {
    ...
    player.gearStats[1].topSpeed = CoverF_Speeds[archetype-1][0] + Archetype_TopSpeedBonus[std::to_underlying(player.characterArchetype)] - typeTopSpeedDebuff;
    if (player.state == PlayerState::Cruise) player.speedCap = CoverF_Speeds[archetype-1][0] + Archetype_TopSpeedBonus[std::to_underlying(player.characterArchetype)] - typeTopSpeedDebuff;
...
void Player_CoverF(Player &player) {
	...
	if(player.input->toggleFaceButtons.hasAny(Buttons::DPadLeft)) {
		...
		player.specialFlags &= ~SpecialFlags::noBoostSpeedBoostChain;
